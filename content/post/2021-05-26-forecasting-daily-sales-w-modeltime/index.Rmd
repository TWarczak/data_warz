...
The [Superstore Sales Dataset](https://www.kaggle.com/rohitsahoo/sales-forecasting/) on Kaggle is relatively unexplored. There's a couple python exploratory data analysis projects posted but nothing using R and only one 7-day forecast using a simple SARIMAX model.      
```{r data, cache=TRUE}
...
```
```{r Sales-by-State-Region, fig.width = 18.5, fig.height = 12.5}
...
```{r Orders-&-Sales-in-log($)-per-Customer-by-Category-Segment, fig.width = 16, fig.height = 10}
...
The `forecast_tbl` has 37 columns and 84 rows for future dates starting 2018-12-31 and ending 2019-03-24. Rows are only missing `sales_trans` data. Good to go.  
...
```
```{r Initial-Cross-Validation-Plan, fig.width = 16, fig.height = 6, message=FALSE}
splits %>%
...
```{r index-19, results='hide', cache=TRUE}
set.seed(321)
...
```{r index-20, results='hide', cache=TRUE}
...
###  PROPHET
*Keep `order_date` feature*
```{r index-21, results='hide', cache=TRUE}
...
```{r index-22, results='hide', cache=TRUE}
...
###  PROPHET BOOST
*Keep `order_date` feature*
```{r index-23, results='hide', cache=TRUE}
...
###  SVM
*Set `order_date` to 'indicator'*
```{r index-24, results='hide', cache=TRUE}
set.seed(321)
...
###  RANDOM FOREST
*Set `order_date` to 'indicator'*
```{r index-25, results='hide', cache=TRUE}
set.seed(321)
...
###  NNET
*Set `order_date` to 'indicator'*
```{r index-26, results='hide', cache=TRUE}
...
###  NNETAR
*Keep `order_date` feature*
```{r index-27, results='hide', cache=TRUE}
set.seed(321)
...
###  MARS
*Set `order_date` to 'indicator'*
```{r index-28, results='hide', cache=TRUE}
...
###  GLMNET
*Set `order_date` to 'indicator'*
```{r index-29, results='hide', cache=TRUE}
...
```{r Models-fit-testing-splits, fig.width = 16, fig.height = 6}
...
```
```{r K-fold-CV-Plan, fig.width = 16, fig.height = 7, message=FALSE}
resamples_kfold %>%
   tk_time_series_cv_plan() %>%
...
resamples_tscv <- time_series_cv(data        = training(splits) %>% drop_na(),
                                 ...
                                 slice_limit = 6)
```
```{r Time_Series-CV-Plan, fig.width = 16, fig.height = 7, message=FALSE}
resamples_tscv %>%
   tk_time_series_cv_plan() %>%
   plot_time_series_cv_plan(order_date,
                            sales_trans,
...
```
```{r index-40, include=FALSE}
# Results Table 1

tune_results_prophet_boost_1 %>% show_best('rmse', n = 10)
```
```{r index-41, fig.width = 20, fig.height = 6, include=FALSE}
# Results Plot 1
...
grid_spec_prophet_boost_2 <- grid_latin_hypercube(changepoint_num(range = c(28,32)),
                                                  ...
                                                  size = 20)
```
```{r index-43, include=FALSE, cache=TRUE}
# Tuning Round 2
...
tune_results_prophet_boost_2 <- wflw_spec_prophet_boost_tune %>% 
   tune_grid(resamples  = resamples_kfold,
             ...
             control    = control_grid(verbose = TRUE))
```
```{r index-44, fig.width = 20, fig.height = 6, include=FALSE}
...
**Tuning Round 1**
```{r index-49, results='hide', cache=TRUE}
set.seed(321)
...
**Results Plot 1**
```{r index-51, out.width='50%', results='hide'}
 tune_results_svm %>% 
     autoplot +
     geom_point(aes(color = '#fed9a6'), show.legend = F) +
     geom_smooth(se = FALSE, size = 0.5) +
...
**Tuning Round 2**
```{r index-53, results='hide', cache=TRUE}
set.seed(321)
...
**Results Plot 2**
```{r index-55, out.width='50%',  results='hide'}
 tune_results_svm_2 %>% 
     autoplot +
     geom_point(aes(color = '#fed9a6'), show.legend = F) +
     geom_smooth(se = FALSE, size = 0.5) +
...
grid_spec_svm_f_1 <- grid_latin_hypercube(parameters(model_spec_svm_tune_f),
                                          size = 20)
```
```{r index-60, results='hide', include=FALSE, cache=TRUE}
# Tuning Round 1
...
```
```{r index-62, fig.width = 8, fig.height = 6, include=FALSE}
# Results Plot
...
grid_spec_svm_f_2 <- grid_latin_hypercube(cost(range = c(1,2),
                                          ...
                                          size = 10)
```
```{r index-64, results='hide', include=FALSE, cache=TRUE}
# Tuning Round 2
set.seed(321)
tune_results_svm_f_2 <- wflw_spec_svm_tune_f %>%
                        tune_grid(resamples  = resamples_kfold,
                                  ...
                                  control    = control_grid(verbose = TRUE))
```
```{r index-65, include=FALSE}
# Results Table 2

tune_results_svm_f_2 %>% show_best('rmse', n = 10)
```
```{r index-66, fig.width = 8, fig.height = 6, include=FALSE}
# Results Plot 2 
...
tune_results_rf <- wflw_spec_rf_tune %>%
                   tune_grid(resamples = resamples_kfold,
                             ...
                             control   = control_grid(verbose = TRUE))
```
```{r index-72, include=FALSE}
# Results Table

tune_results_rf %>% show_best('rmse', n = 10)
```
```{r index-73, fig.width = 8, fig.height = 6, include=FALSE}
# Results Plot
 ...
 tune_results_rf %>% 
     ...
     theme_dark_grey() 
```
```{r index-74, include=FALSE}
# Tuning Grid 2 
grid_spec_rf_2 <- grid_latin_hypercube(trees(range = c(1500,1800)),
                                       min_n(range = c(35,50)),
                                       size = 10)
```
```{r index-75, include=FALSE, cache=TRUE}
# Tuning Round 2 
...
tune_results_rf_2 <- wflw_spec_rf_tune %>%
                     tune_grid(resamples = resamples_kfold,
                               ...
                               control   = control_grid(verbose = TRUE))
```
```{r index-76, include=FALSE}
# Results Table 2 

tune_results_rf_2 %>% show_best('rmse', n = 10)

```
```{r index-77, fig.width = 8, fig.height = 6, include=FALSE}
# Results Table 2
 ...
 tune_results_rf_2 %>% 
     ...
     theme_dark_grey() 
```
```{r index-78, include=FALSE, cache=TRUE}
# Fit Tuned Workflow on Original Traning Data
...
tune_results_nnetar <- wflw_spec_nnetar_tune %>% 
                       tune_grid(resamples = resamples_tscv,
                                 ...
                                 control   = control_grid(verbose = TRUE))
```
```{r index-83, include=FALSE}
tune_results_nnetar %>% show_best('rmse', n = 10)
```
```{r index-84, fig.width = 14, fig.height = 6, include=FALSE}
 tune_results_nnetar %>% 
     autoplot +
     geom_point(aes(color = '#fed9a6'), show.legend = F) +
     geom_smooth(se = FALSE, size = 0.5) +
     theme_dark_grey() 
```
```{r index-85, cache=TRUE, include=FALSE}
set.seed(321)
grid_spec_nnetar_2 <- grid_latin_hypercube(non_seasonal_ar(range = c(1,1)),
                                           ...
                                           size = 10)
```
```{r index-86, include=FALSE, cache=TRUE}
set.seed(321)
tune_results_nnetar_2 <- wflw_spec_nnetar_tune %>% 
                         tune_grid(resamples = resamples_tscv,
                                   ...
                                   control   = control_grid(verbose = TRUE))
```
```{r index-87, include=FALSE}
tune_results_nnetar_2 %>% show_best('rmse', n = 10)
```
```{r index-88, fig.width = 8, fig.height = 6, include=FALSE}
#Results Plot 2
...
**Tuning Round 1**
```{r index-93, results='hide', cache=TRUE}
set.seed(321)
...
```{r Tuned-Models-fit-testing-splits, fig.height=6, fig.width=16}
...
```{r index-105, fig.height=8, fig.width=16}
refit_tbl %>% 
...
```{r index-106, fig.height=8, fig.width=16}
refit_tbl %>% 
   modeltime_forecast(new_data    = forecast_tbl,
                      actual_data = data_prepared_tbl,
                      keep_data   = FALSE) %>%  
