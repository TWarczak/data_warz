...
date: '2021-05-20'
categories: [R]
description: In my introductory blog post, I use the time-series modeling package {modeltime} and a Kaggle sales dataset to forecast 3 months of daily sales. This is a {tidymodels} approach with with a {modeltime} twist.
...
```
```{r Sales by State/Region, fig.width = 18.5, fig.height = 12.5, cache=TRUE}
...
```{r Orders & Sales in log($) per Customer by Category/Segment, fig.width = 16, fig.height = 10, cache=TRUE}
...
```{r Daily Total Sales, fig.width = 16, fig.height = 6}
sales_daily_tbl %>% 
...
```{r Daily Total Sales (Padded & Transformed), fig.width = 16, fig.height = 6}
...
###  ACF/PACF
```{r Lag Diagnostics, fig.width = 16, fig.height = 8}
...
```{r Lagged Rolling Averages, fig.width = 16, fig.height = 6}
...
```{r, include=FALSE}
skimr::skim_without_charts(full_data_prepared_tbl)
```
...
```{r Models fit on testing(splits), fig.width = 16, fig.height = 6}
...
```{r Tuned Models fit on testing(splits), fig.height=6, fig.width=16}
...
```{r Ensemble Forecast, fig.height=8, fig.width=16}
refit_weight_tbl %>% 
   modeltime_forecast(new_data    = forecast_tbl,
                      actual_data = data_prepared_tbl)  %>%  
