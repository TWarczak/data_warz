...
-   `image_name`  - Location of built-in XGBoost docker container image in AWS
...
session$describe_training_job(job_name_1)[["TrainingJobStatus"]]
```
...
session$describe_transform_job(job_name_1)[["TransformJobStatus"]]
```
...
job_name_2 <- paste(project, algo, timestamp, sep = "-")
s3_train_input <- sagemaker$inputs$TrainingInput(s3_data = s3_train,
                                                 content_type = 'csv')
s3_valid_input <- sagemaker$inputs$TrainingInput(s3_data = s3_validation,
                                                 content_type = 'csv')
...
session$describe_tuning_job(job_name_2)[["HyperParameterTuningJobStatus"]]
```

### Step 10 - Evaluate tuning job results

```{r chunk38, cache=TRUE}
tuning_job_results <- sagemaker$HyperparameterTuningJobAnalytics(job_name_2)
...
The next plot shows how the Bayesian optimizer focused on the region of the search
...
```{r chunk48, eval=FALSE, cache=TRUE, include=FALSE}
...
wins. My submission scored **0.07847**, which would have been 10th place (of 36
...
model, we could log transform numeric variables, down-sample to balance the data,
and/or remove the low-performing features. There should be room to improve model
performance.
